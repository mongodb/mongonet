...
func getTestClient(host string, port int, secondaryReads bool, appName string) (*mongo.Client, error) {
...
func insertDummyDocs(host string, proxyPort, numOfDocs int) error {
	client, err := getTestClient(host, proxyPort, false, "insertDummyDocs")
	if err != nil {
		return err
	}
