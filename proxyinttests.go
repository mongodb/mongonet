...
func DoConcurrencyTestRun(logger *slogger.Logger,
	hostname string, mongoPort, proxyPort int,
	...
	if preSetupFunc != nil {
		err = preSetupFunc(logger, hostname, mongoPort, proxyPort)
		if err != nil {
	...
	if setupFunc != nil {
		err = setupFunc(logger, hostname, mongoPort, proxyPort)
		if err != nil {
	...
	defer cleanupFunc(logger, hostname, mongoPort, proxyPort)
	...
	for j := 0; j < iterations; j++ {
		...
		for i := 0; i < workers; i++ {
			...
			go func(num int, wg *sync.WaitGroup) {
				...
				if testFunc != nil {
					elapsed, success, err = testFunc(logger, hostname, mongoPort, proxyPort, num, j)
